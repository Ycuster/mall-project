...
export const useCartStore = defineStore('cart', () => {
  ...
  async function fetch(): Promise<void> {
    loading.value = true
    const res = await request.get<CartItem[]>('/cart')
    if (res.code === 200) items.value = res.data
    loading.value = false
  }

  async function add(productId: number | string, quantity: number = 1): Promise<boolean> {
    const res = await request.post<null>('/cart', { product_id: Number(productId), quantity })
    if (res.code === 200) {
  ...
  async function updateQuantity(id: number, quantity: number): Promise<void> {
    await request.put<null>(`/cart/${id}`, { quantity })
    await fetch()
  }

  async function remove(id: number): Promise<void> {
    await request.delete<null>(`/cart/${id}`)
    await fetch()
  }

  async function clear(): Promise<void> {
    await request.delete<null>('/cart')
    items.value = []
  }
