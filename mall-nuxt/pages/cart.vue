  ...
  <div class="page-container">
    <el-card shadow="never">
      ...
      <el-empty v-if="!cartStore.items.length" description="购物车是空的">
        <NuxtLink to="/products"><el-button type="primary">去选购</el-button></NuxtLink>
      </el-empty>
          ...
          <el-table-column label="商品" min-width="300">
            <template #default="{ row }">
              <div style="display: flex; align-items: center; gap: 12px">
                ...
                />
                <div>
                  <NuxtLink :to="'/product/' + row.product_id" style="font-weight: 500">
                    {{ row.name }}
                  </NuxtLink>
          ...
          <el-table-column label="操作" width="80" align="center">
            <template #default="{ row }">
              <el-popconfirm title="确定移除该商品？" @confirm="cartStore.remove(row.id)">
                <template #reference>
                  <el-button type="danger" text><ClientOnly><el-icon><Delete /></el-icon></ClientOnly></el-button>
                </template>
              </el-popconfirm>
            </template>
          </el-table-column>
