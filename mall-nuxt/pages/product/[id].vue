<template>
  <div class="page-container" v-loading="!product && loading">
        ...
        <el-row :gutter="40">
          ...
          <el-col :md="14" :sm="24">
            ...
            <el-button type="primary" size="large" style="width: 240px; height: 48px; font-size: 1rem" @click="handleAdd">
              <DelayedRender><el-icon :size="20"><ShoppingCart /></el-icon></DelayedRender> 加入购物车
            </el-button>

            <div style="margin-top: 20px; display: flex; gap: 24px">
              <el-text type="info"><DelayedRender><el-icon><Shield /></el-icon></DelayedRender> 正品保证</el-text>
              <el-text type="info"><DelayedRender><el-icon><Van /></el-icon></DelayedRender> 全场包邮</el-text>
              <el-text type="info"><DelayedRender><el-icon><RefreshRight /></el-icon></DelayedRender> 7天退换</el-text>
            </div>
          </el-col>
        </el-row>
      </el-card>
    ...
    <el-empty v-else-if="!loading" description="商品不存在">
      <NuxtLink to="/products"><el-button type="primary">返回商品列表</el-button></NuxtLink>
    </el-empty>
  </div>
</template>
...
await useAsyncData(`product-${productId.value}`, async () => {
  try {
    ...
    const res = await $api.get<Product>('/products/' + productId.value)
    if (res.code === 200) {
